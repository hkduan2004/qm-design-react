...
// ====获取LODOP对象的主过程：====
export function getLodop(oOBJECT, oEMBED, LICENSES = []) {
  ...
  try {
    ...
    if (needCLodop()) {
      ...
      if (!LODOP) {
        notification.warning({
          message: 'CLodop 信息',
          duration: null,
          description: strCLodopInstall_1 + (CLodopIsLocal ? strCLodopInstall_2 : '') + strCLodopInstall_3,
        });
        return;
      } else {
        // eslint-disable-next-line no-undef
      ...
      if (!LODOP || !LODOP.VERSION) {
        ...
        if (ua.indexOf('Firefox') >= 0) {
          notification.warning({
            message: 'CLodop 信息',
            duration: null,
            description: strHtmFireFox,
          });
        }
        notification.warning({
          message: 'CLodop 信息',
          duration: null,
          description: is64IE ? strHtm64_Install : strHtmInstall,
        });
        return LODOP;
      }
    }
    if (LODOP.VERSION < '6.2.2.6') {
      if (!needCLodop()) {
        notification.warning({
          message: 'CLodop 信息',
          description: is64IE ? strHtm64_Update : strHtmUpdate,
        });
      }
    }
